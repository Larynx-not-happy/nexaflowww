...
test("all pages, metadata, accessibility, assets and internal links", async ({ page, request }) => {
  ...
  for (const asset of [
    ...
    expect((await request.get(base + asset)).status(), asset).toBe(200);
...
test("pricing, FAQ, keyboard, consent and mobile layouts", async ({ page }) => {
  ...
  page.on("request", (r) => {
    if (/google-analytics|googletagmanager/.test(r.url())) tracking.push(r.url());
  });
  await page.goto(base + "/pricing");
  await page.getByRole("button", { name: "Essential only" }).click();
  await page.getByRole("button", { name: "Yearly (save ~20%)" }).click();
  await expect(page.getByText("$180 per seat / year, billed annually")).toBeVisible();
  await page.getByRole("link", { name: "Ask about Pro" }).click();
  await expect(page.locator("#message")).toHaveValue("I'm interested in the pro plan. ");
  await expect(page.locator("#topic")).toHaveValue("sales");
  await page.goto(base + "/faq");
  await page.waitForLoadState("networkidle");
  const question = page.getByRole("button", { name: "What is NexaFlow?" });
  await question.focus();
  await page.keyboard.press("Enter");
  await expect(question).toHaveAttribute("aria-expanded", "true");
  await page.getByRole("button", { name: "Cookie preferences" }).click();
  await page.getByRole("button", { name: "Accept analytics" }).click();
  expect(await page.evaluate(() => localStorage.getItem("nexaflow.cookie-consent.v1"))).toBe(
    "accepted",
  );
...
test("contact validates, submits once and handles network errors", async ({ page }) => {
  ...
  await page.locator("#name").fill("NexaFlow QA");
  await page.locator("#email").fill("qa-nexaflow@example.com");
  await page.locator("#topic").selectOption("other");
  await page
    .locator("#message")
    .fill("Automated QA submission: verifying private contact storage.");
