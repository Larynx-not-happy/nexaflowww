...
export function getConsent(): ConsentValue | null {
  ...
  } catch { /* Storage may be blocked; default to no consent. */ }
  return consent;
}
...
export function setConsent(value: ConsentValue) {
  consent = value;
  try { localStorage.setItem(CONSENT_STORAGE_KEY, value); } catch { /* Session-only choice. */ }
...
export function trackPageView(path: string) {
  ...
  lastPath = path;
  window.gtag?.('event', 'page_view', { page_path: path, page_location: window.location.origin + path, page_referrer: '', page_title: document.title });
}
